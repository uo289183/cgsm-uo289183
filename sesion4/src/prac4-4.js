...
if (!isWebGL2Available()) {
    ...
    audioLoader2.load("audio/suprise.mp3", (buffer) => {
    ...
    // Raycaster for object selection and collision detection
    const rayCaster = new THREE.Raycaster();
    const mouse = new THREE.Vector2();
    let intersectedObject = null;

    // Possible camera movement directions in the XZ plane (normalized)
    const movements = [
    ...
    const collisionDistance = 20; // Maximum distance to consider a collision
    ...
    function animate() {
        ...
        // Collision detection: cast rays in all 8 directions from camera position
        let collision = false;
        ...
        if (collision) {
            controls.update(-delta); // Undo last movement to prevent traversal
        }

        // Update intersected object via raycasting (for selection)
